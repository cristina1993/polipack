...
$id = $_REQUEST[id];
$rst1 = pg_fetch_array($Set->lista_una_etiqueta($id));
if ($rst1[etg_tipo] == 0) {
    $core = $rst1[pro_propiedad5];
} else {
    $core = $rst1[pro_capa];
}
$datos = "$rst1[etg_numero],$rst1[pro_ancho],$rst1[etg_peso_bruto],$rst1[pro_espesor],$rst1[pro_largo],$rst1[etg_pallet1],$rst1[etg_estado],$rst1[etg_tamano],$rst1[etg_copias],$rst1[etg_fecha],$rst1[cli_raz_social],$core,$rst1[etg_peso_neto],$rst1[etg_espacio8],$rst1[etg_operador],$rst1[etg_observaciones]";
$dt = $datos;
$data = explode(',', $dt);
if ($data[6] == 3) {
    $rst = pg_fetch_array($Set->lista_una_conf_etiqueta(2));
} else {
    $rst = pg_fetch_array($Set->lista_una_conf_etiqueta($data[7]));
}
...
//        }
        if ($dt[10] == 1) {
            ...
            $this->SetFont('helvetica', '', 7);
            $this->Cell(12, 5, $data[11] . ' kg', 'RTB', 0, 'L');
        } else {
        ...
        if ($dt[11] == 1) {
            ...
            $this->SetFont('helvetica', '', 7);
            $this->Cell(14, 5, $data[2] . ' Kg', 'RTB', 0, 'L');
        } else {
            $this->Cell(17, 5, '', 'LTB', 0, 'L');
            $this->Cell(14, 5, '', 'RTB', 0, 'L');
        }
        if ($dt[12] == 1) {
            ...
            $this->SetFont('helvetica', '', 7);
            $this->Cell(12, 5, round($data[12], 2) . ' Kg', 'RTB', 0, 'L');
        } else {
            $this->Cell(12, 5, '', 'LTB', 0, 'L');
        ...
        if ($dt[13] == 1) {
            ...
            $this->SetFont('helvetica', '', 7);
            $this->Cell(14, 5, $data[3] . ' micra', 'RTB', 0, 'L');
        } else {
            $this->Cell(17, 5, '', 'LTB', 0, 'L');
            $this->Cell(14, 5, '', 'RTB', 0, 'L');
        }
...
//        }
        ...
        if ($dt[24] == 1) {
            ...
            $this->Cell(55, 28, '', '1', 0, 'L');
        } else {
            $this->Cell(55, 28, '', 'LTB', 0, 'L');
        }
        $this->Ln();
//        $this->SetXY(26, 69);
//        if ($dt[25] == 1) {
...
$pdf = new PDF($orientation = $o, $unit = 'mm', $size = $tam);
$n = 0;
while ($n < $data[8]) {
    $pdf->AddPage();
    if ($rst[eti_tamano] != 3) {
