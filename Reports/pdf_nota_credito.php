...
class PDF extends FPDF {
    ...
    function factura($rst, $cns, $emisor) {
        ...
        $this->Ln();
        $this->Cell(195, 5, "FECHA EMISION : " . $rst[ncr_fecha_emision], 'LR', 0, 'L');
        $this->Ln();
        ...
        $this->Ln();
        $this->Cell(195, 5, "FECHA DE EMISION (Comprobante a modificar) : " . $rst[ncr_fecha_emi_comp], 'LR', 0, 'L');
        $this->Ln();
        ...
        while ($rst2 = pg_fetch_array($cns)) {
                $rst_prod= pg_fetch_array($Set->lista_un_prod_id($rst2[pro_id],$rst2[dnc_tab]));
            $n++;
            $this->SetXY(3, 130 + $y);
            $this->SetFont('helvetica', '', 8);
            $this->Cell(25, 5, $rst2[dnc_codigo], 1, 0, 'C');
            $this->Cell(25, 5, $rst_prod[cod_aux], 1, 0, 'C');
...
if ($val == 1) {
    ...
    $pdf->Output('../mail/' . $rst[ncr_clave_acceso] . '.pdf', 'F');
    include_once '../Clases/clsMail.php';
    $Obj = new Mail();
    $files = array('../mail/' . $rst[ncr_clave_acceso] . '.pdf', '../xml_docs/' . $rst[ncr_clave_acceso] . '.xml');
    $sms = $Obj->envia_correo(strtolower($rst[ncr_email]), strtoupper($rst[ncr_nombre]), $files, $rst[ncr_numero], 4);
    echo $sms;
} else {
    if (!isset($_GET[ide])) {
        $pdf->factura($rst, $cns, $emisor);
        ...
        $pdf->factura($rst, $cns, $emisor);
        $pdf->Output($rst[ncr_numero] . '.pdf', 'D');
    }
}
