...
class PDF extends FPDF {
    ...
    function factura($rst, $cns, $emisor, $rst1, $g, $cns1, $rst2, $emisor_local) {
        ...
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 19);
        $this->Cell(85, 5, "  No.   " . $rst1[num_documento], 'LR');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 24);
        ...
        while ($rst = pg_fetch_array($cns)) {
            ...
            $this->Cell(89, 5, $rst[lote] . " " . substr($rst[descripcion], 0, 40), 1, 0, 'C');
    ...
    function no_exite($ndoc) {
        $this->SetFont('helvetica', 'B', 10);
        $this->Ln($x + 4, $y + 90);
        $this->Image('../img/logo_noperti.jpg', 5, 1, 100);
        $this->Image('../img/error.png', 5, 30);
        $this->Text(20, 40, 'CI/RUC ' . $_GET[ci] . ' o FACTURA ' . $ndoc . ' INCORRECTOS REVICE Y VUELVA A INTERTARLO');
    }
...
if (isset($_GET[ide])) {
    ...
    $pdf->Output($rst1[num_documento] . '.pdf', 'D');
} else {
    $pdf->factura($rst, $cns, $emisor, $rst1, $g, $cns1, $rst2, $emisor_local);
