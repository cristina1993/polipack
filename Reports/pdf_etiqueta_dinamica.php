...
//                break;
    ...
    function etq_grande($data, $rst, $lg) {
...
//        }
        ...
        if ($dt[14] == 1) {
            ...
            $this->SetFont('helvetica', '', 7);
            $this->Cell(12, 5, $data[1] . ' mm', 'RTB', 0, 'L');
        } else {
            $this->Cell(12, 5, '', 'LTB', 0, 'L');
            $this->Cell(12, 5, '', 'RTB', 0, 'L');
        }
...
while ($m < $j) {
    ...
    if ($data[6] == 3) {
        $rst = pg_fetch_array($Set->lista_una_etiqueta(2));
        $tam = 'etiqueta_reg';
        $o = 'p';
    }

    while ($n < $data[8]) {
        $pdf->AddPage($orientation = $o, $size = $tam);
        if ($rst[eti_tamano] != 3) {
            $pdf->etq($data, $rst);
        } else {
            $pdf->etq_grande($data, $rst, $lg);
        }
        $n++;
    }
    $m++;
