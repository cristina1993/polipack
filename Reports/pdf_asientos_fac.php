...
class PDF extends FPDF {
    ...
    function encabezado_tab($as) {
        ...
        $rst_fac = pg_fetch_array($Set->listar_factura_asiento($as));
        $rst_ret = pg_fetch_array($Set->listar_retencion_regid($rst_fac[reg_id]));
        $this->SetFont('Arial', '', 8);
        $this->Cell(100, 5, "FECHA: " . $rst1[con_fecha_emision], 'T', 0, 'L');
        $this->Cell(100, 5, "ASIENTO: " . $as, 'T', 0, 'L');
        $this->Ln();
        $this->Cell(200, 5, "CLIENTE: " . $rst_fac[cli_raz_social], 0, 0, 'L');
        $this->Ln();
    ...
    function asientos($as, $emisor, $desde, $hasta) {
        ...
        while ($rst_cuentas = pg_fetch_array($cns_cuentas)) {
            ...
            if ($rst_cuentas[tipo] == 0) {
                ...
                $debe = 0;
                if (!empty($rst_v[haber])) {
                    $haber = $rst_v[haber] - $rst_v[debe];
                } else {
