...
$rst = pg_fetch_array($Set->lista_una_guia_id($id));
...
if ($x == 1) {
    ...
    $pdf->Output('../mail/' . $rst[gui_clave_acceso] . '.pdf', 'F');
    include_once '../Clases/clsMail.php';
    $Obj = new Mail();
    $files = array('../mail/' . $rst[gui_clave_acceso] . '.pdf', '../xml_docs/' . $rst[gui_clave_acceso] . '.xml');
    $sms = $Obj->envia_correo(strtolower($rst[cli_email]), strtolower($rst[gui_nombre]), $files, $id, 6);
    echo $sms;
} else {
    if (!isset($_GET[ide])) {
        $pdf->factura($rst, $cns, $emi);
        ...
        $pdf->factura($rst, $cns, $emi);
        $pdf->Output($rst[gui_numero] . '.pdf', 'D');
    }
}
