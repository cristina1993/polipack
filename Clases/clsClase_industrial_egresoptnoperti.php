...
class Clase_industrial_egresopt {
    ...
    function lista_buscador_industrial_egresopt($txt) {
        if ($this->con->Conectar() == true) {
            return pg_query("SELECT * FROM erp_i_pedido_producto_terminado m, erp_i_productos p,erp_transacciones t, erp_i_cliente c where p.pro_id= m.pro_id and m.trs_id=t.trs_id and m.cli_id=c.cli_id and m.trs_id=25 $txt  order by m.ped_documento desc");
        }
    }
