...
class Clase_factura {
    ...
    function lista_no_enviados() {
        if ($this->con->Conectar() == true) {
            return pg_query("SELECT * FROM  erp_nota_credito  where (ncr_estado_correo<>'ENVIADO' or ncr_estado_correo is null) and char_length(ncr_autorizacion)=37  and ncr_nombre<>'CONSUMIDOR FINAL' order by ncr_id desc limit 1");
        }
    }
    ...
    function update_estado_correo($sts, $id) {
        if ($this->con->Conectar() == true) {
            return pg_query("UPDATE erp_nota_credito SET ncr_estado_correo='$sts' where ncr_id=$id ");
        }
    }
...
//        $Set = new Clase_nota_Credito();
        ...
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 19);
        $this->Cell(85, 5, "  No.   " . $rst[ncr_numero], 'LR');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 24);
        ...
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 29);
        $this->Cell(85, 5, $rst[ncr_autorizacion], 'LR');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 34);
        ...
        $this->Cell(85, 5, "AUTORIZACION : " . $rst[ncr_fec_hora_aut], 'LR');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 44);
        ...
        $this->SetFont('helvetica', 'B', 10);
        $this->SetXY(115, 59);
        $this->Code39($x + 115, $y + 59, $rst[ncr_clave_acceso]);
        ...
        $this->Ln($x + 4, $y + 1);
        $this->SetFont('helvetica', 'B', 8);
        $this->Cell(140, 5, "Razon Social / Nombres y Apellidos : " . utf8_decode($rst[ncr_nombre]), 'LT', 0, 'L');
        $this->Cell(55, 5, "Identificacion : " . utf8_decode($rst[ncr_identificacion]), 'RT', 'R');
        $this->Ln();
        $this->Cell(195, 5, "Fecha de Emision : " . $rst[ncr_fecha_emision], 'LR', 0, 'L');
        $this->Ln();
        ...
        $this->Ln();
        $this->Cell(195, 5, "Comprobante que se modifica : " . $rst[ncr_num_comp_modifica], 'LR', 0, 'L');
        $this->Ln();
        $this->Cell(195, 5, "Fecha Emision (Comprobante a modificar) : " . $rst[ncr_fecha_emi_comp], 'LR', 0, 'L');
        $this->Ln();
        $this->Cell(195, 5, "Razon de Modificacion : " . $rst[ncr_motivo], 'LBR', 0, 'L');
        $this->Ln();
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
        //////////////////////////////////////////////// CUERPO ////////////////////////////////////////////////////////                        
        $this->Ln();
        ...
        while ($rst2 = pg_fetch_array($cns)) {
            ...
            $this->Cell(25, 5, $rst2[dnc_codigo], 1, 0, 'C');
            $this->Cell(25, 5, '', 1, 0, 'C');
            $this->Cell(16, 5, $rst2[dnc_cantidad], 1, 0, 'C');
            $this->Cell(85, 5, $rst_lote[dnc_lote] . " " . substr($rst2[dnc_descripcion], 0, 37), 1, 0, 'L');
            $this->Cell(14, 5, $rst2[dnc_porcentaje_descuento], 1, 0, 'R');
            $this->Cell(20, 5, $rst2[dnc_precio_unit], 1, 0, 'R');
            $this->Cell(20, 5, $rst2[dnc_precio_total], 1, 0, 'R');
            $this->Ln();
            $y = $y + 5;
        }
        //___________________________________________________________________________
        ...
        $this->Cell(20, 5, $rst[ncr_subtotal12], 'LRTB', 0, 'R');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 10);
        ...
        $this->Cell(20, 5, $rst[ncr_subtotal0], 'LRTB', 0, 'R');
        $this->Ln();
        $this->SetFont('helvetica', '', 8);
        ...
        $this->Cell(20, 5, $rst[ncr_subtotal_no_iva], 'LRTB', 0, 'R');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 8);
        $this->Cell(85, 5, "Direccion : " . $rst[ncr_direccion], 'LR', 0, 'L');
        $this->Cell(62, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->SetFont('helvetica', '', 8);
        $this->Cell(34, 5, "SUBTOTAL Exento IVA", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[ncr_subtotal_ex_iva], 'LRTB', 0, 'R');
        $this->Ln();
        $this->SetFont('helvetica', '', 8);
        $this->Cell(85, 5, "", 'LR', 0, 'L');
        $this->Cell(62, 5, "", '', 0, 'L');
        ...
        $this->Cell(20, 5, str_replace(',', '', number_format($rst[ncr_subtotal_ex_iva] + $rst[ncr_subtotal_no_iva] + $rst[ncr_subtotal0] + $rst[ncr_subtotal12], 4)), 'LRTB', 0, 'R');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 8);
        $this->Cell(85, 5, "Telefono : " . $rst[nrc_telefono], 'LR', 0, 'L');
        $this->Cell(62, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->Cell(34, 5, "TOTAL Descuento", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[ncr_total_descuento], 'LRTB', 0, 'R');
        $this->Ln();
        ...
        $this->Cell(20, 5, $rst[ncr_total_ice], 'LRTB', 0, 'R');
        $this->Ln();
        $this->SetFont('helvetica', 'B', 8);
        $this->Cell(85, 5, "Email :  " . strtolower($rst[ncr_email]), 'LRB', 0, 'L');
        $this->Cell(62, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->Cell(34, 5, "IVA 12%", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[ncr_total_iva], 'LRTB', 0, 'R');
        $this->Ln();
        $this->Cell(147, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->Cell(34, 5, "IRBPNR", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[ncr_irbpnr], 'LRTB', 0, 'R');
        $this->Ln();
        $this->Cell(147, 5, "", '', 0, 'L');
        $this->SetFont('helvetica', '', 8);
        $this->Cell(34, 5, "VALOR TOTAL", 'LRTB', 0, 'L');
        $this->Cell(20, 5, $rst[nrc_total_valor], 'LRTB', 0, 'R');
        $this->Ln();
        ////////////////////////////////////////////////////////////////////////////////////////////////////////////////
    }
...
while ($rst = pg_fetch_array($cns_docs)) {
    ...
    $rstnc = pg_fetch_array($Set->lista_una_nota_credito_id($id)); //lista nota
    $emisor = pg_fetch_array($Set->lista_emisor($emi));
    $cns = $Set->lista_detalle_nota_credito($id);
    $PDF->AddPage();
    $PDF->factura($rstnc, $cns, $emisor);
//    $PDF->Output();
    $PDF->Output('../mail/' . $id . '.pdf', 'F');
    $doc = $Set->recupera_datos($rst[ncr_clave_acceso], $amb);
    $fch = fopen("../xml_docs/" . $rst[ncr_clave_acceso] . ".xml", "w+o");
    fwrite($fch, $doc[4]);
    fclose($fch);
    $files = array('../mail/' . $id . '.pdf', '../xml_docs/' . $rst[ncr_clave_acceso] . '.xml');
    echo $resp = $Obj->envia_correo($rst[ncr_email], $rst[ncr_nombre], $files, $rst[ncr_numero], 4);
    if ($resp == 0) {
