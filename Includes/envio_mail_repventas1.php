...
include_once '../Clases/clsMail_rep_venta.php';
...
$rst_correo = pg_fetch_array($Set->lista_configuraciones());
$correo = $rst_correo[con_correo];
if ($rst[con_fecha] != $fec) {
    $pdf->Output('../mail/rep_ventas_' . $fec . '.pdf', 'F');
    $files = array('../mail/rep_ventas_' . $fec . '.pdf');
    $resp = $Obj->envia_correo_venta($files, $correo);
}
