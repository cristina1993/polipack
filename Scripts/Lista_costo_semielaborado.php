...
session_start();
set_time_limit(0);
include_once '../Clases/clsClase_costo_semielaborado.php';
include_once '../Includes/permisos.php';
$Set = new Clase_costo_semielaborado();
if (isset($_GET[search])) {
    $desde = $_GET[desde];
    $hasta = $_GET[hasta];
    $txt = strtoupper($_GET[txt]);
    if (!empty($txt)) {
        $text = "where (ord_num_orden like '%$txt%'
        ...
        <script>
            ...
            function save(id, c, t, f, cod, n) {
                ...
                $.ajax({
                    ...
                    data: {op: 0, 'data[]': data, id: id, t: t, 'fields[]': fields, mod: 1}, //op sera de acuerdo a la acion que le toque
                    success: function (dt) {
                        dat = dt.split('&');
                        if (dat[0] == 0) {
                            parent.document.getElementById('mainFrame').src = '../Scripts/Lista_costo_semielaborado.php?search=1&desde=<?php echo $desde ?>&hasta=<?php echo $hasta ?>&txt=<?php echo $txt ?>';
                        } else {
                            alert(dt);
                        }
                    }
    ...
    <body>
        ...
        <table style="width:100%" id="tbl">
            <caption  class="tbl_head">
                ...
                <center class="cont_title" >LISTA COSTO EXTRUSION</center>
            ...
            <tbody id="tblbody">
                ...
                while ($rstPedido = pg_fetch_array($cnsPedido)) {
                    ...
                    $rst_cmp = pg_fetch_array($Set->lista_costo_mp($rstPedido['ord_num_orden'], $rstPedido['fec_fin']));
                    $rst_cins = pg_fetch_array($Set->lista_costo_insumos($rstPedido['ord_num_orden'], $rstPedido['fec_fin']));
                    ...
                    <tr>
                        ...
                        <td align="left"><?PHP echo $rstPedido['ord_num_orden'] ?></td>   
                        ...
                        if ($estado == 'SIN REVISAR'|| $estado == 'ACTUALIZAR') {
                            ?>
                            <td align="center" onclick="save('<?PHP echo $rstPedido[pro_id] ?>', '<?PHP echo $cunit ?>', '1', '<?PHP echo $rstPedido[fec_fin] ?>', '<?PHP echo $rstPedido[pro_codigo] ?>',<?PHP echo $n ?>)" <?PHP echo $style ?>><?PHP echo $estado ?></td>
                            <?PHP
                        } else {
                            ?>
