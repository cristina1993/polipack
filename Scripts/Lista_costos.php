        ...
        <script>
            ...
            function save(id, c, t, cod) {
                ...
                $.ajax({
                    ...
                    data: {op: 11, 'data[]': data, id: id, 'fields[]': fields, t: t}, //op sera de acuerdo a la acion que le toque
                    success: function (dt) {
                        dat = dt.split('&');
                        if (dat[0] == 0) {
                            $('#pre_precio' + c).val(parseFloat(dat[1]).toFixed(4));
                            $('#pre_preciox' + c).val(parseFloat(dat[2  ]).toFixed(4));
                    ...
                    if (msm == true) {
                        $('.precios2').each(function () {
                            ...
                            if ($('#pre_preciox' + i).val() != 0) {
                                ...
                                $.post("actions_preciospt.php", {op: 10, id: id, 'fields[]': fields}, function (dt) {
                                    if (dt == 0) {
                ...
                <tbody id="tbody">
                    ...
                    while ($rst = pg_fetch_array($cns)) {
                        ...
                        if ($f == 0) {
                            ...
                            $rst['pre_costo1'] = $rst1['pre_costo1'];
                            $rst['pre_costo2'] = $rst1['pre_costo2'];
                        } else {
                            if ($rst['pro_tabla'] == '0') {
                                $rst['pro_tabla'] = 'INDUSTRIAL';
                                ...
                                $rst['pre_costo1'] = $rst2['pre_costo1'];
                                $rst['pre_costo2'] = $rst2['pre_costo2'];
                                $tabla = 1;
                            }
                        }
                        if ($rst[pre_id] != '') {
                            ...
                            <tr>
                                ...
                                <td  align="center"><input type ="text" size="10"  id="<?php echo 'pre_precio' . $n ?>"  value="<?php echo number_format($rst['pre_costo1'], 4) ?>" style="text-align:right" disabled /><input type="radio" class="precios1" id="<?php echo 'pre_vald_precio1' . $n ?>" name="<?php echo 'pre_precios' . $n ?>" value="" hidden></td>
                                <td  align="center"><input type ="text" size="10"  id="<?php echo 'pre_preciox' . $n ?>"  value="<?php echo number_format($rst['pre_costo2'], 4) ?>" style="text-align:right" disabled /><input type="radio" class="precios2" id="<?php echo $rst[pre_id] ?>" name="<?php echo 'pre_precios' . $n ?>" value="" lang="<?php echo $n ?>" hidden></td>
