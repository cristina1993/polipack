...
include_once '../Clases/clsClase_movimiento_nopertipt.php';
include_once('../Clases/clsAuditoria.php');
$Clase_movimiento_nopertipt = new Clase_movimiento_nopertipt();
$Adt = new Auditoria();
$op = $_REQUEST[op];
$data = $_REQUEST[data];
$id = $_REQUEST[id];
switch ($op) {
    case 0:
        ...
        if (empty($id)) {
            ...
            while ($n < count($data)) {
                ...
                if ($dt[0] != 'undefined') {
                    if ($Clase_movimiento_nopertipt->insert_movimiento_noperti($dat) == FALSE) {
                        $sms = pg_last_error();
                    } else {
    ...
    case 1:
        ...
        break;
    case 2:
        $rst = pg_fetch_array($Clase_movimiento_nopertipt->lista_un_producto_movimiento($id));
        $id = $rst[pro_id];
        $cod = $rst[pro_codigo];
        $uni = $rst[pro_uni];
        ...
        break;
    case 3:
        $cns = $Clase_movimiento_nopertipt->lista_movimiento_producto($id);
    ...
    case 4:
        $rst = pg_fetch_array($Clase_movimiento_nopertipt->lista_secuencial_movimiento());
    ...
    case 5:
        $rst_cl = pg_fetch_array($Clase_movimiento_nopertipt->lista_un_cliente_movimiento($id));
