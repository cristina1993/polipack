...
if (isset($_GET[tipo]) && $_GET[tipo] != '') {
    ...
    $rst_head = pg_fetch_array($Set->lista_one_by_table_code_finder($txt, $tbl));
    $tipo = $rst_head[ids];
    $cns = $Set->lista_one_by_table_code_finder($txt, $tbl);
    $head = pg_fetch_array($Set->lista_one_data($tbl_set, $tipo));
    $tp_prod = explode('&', $head[1]);
}
?>
            ...
            function del(id, tbl, dat)
            {
                ...
                if (r == true) {
                    $.post("actions.php", {act: 6, tbl: tbl, id: id, 'data[]': data}, function (dt) {
                        if (dt == 0)
                        {
                            parent.document.getElementById('mainFrame').src = '../Scripts/Lista_insumos.php';
                        } else {
                            alert(dt);
                        }
                    });
        ...
        <table style="width:100%" id="tbl">
            <caption class="tbl_head" >
                ...
                <center class="cont_finder">
                    ...
                    <?php }
                    ?>
                    <form style="margin:0px 5px 10px 0px;" id="frmSearch" name="frm1" action="<?php echo $_SERVER['PHP_SELF']; ?>" >
                        <?php
                        if ($Prt->add == 0) {
                            ?>
                            <a href="#" class="btn" title="Nuevo Registro" onclick="auxWindow(0)" >Nuevo</a>
                            <?php
                        }
                        ?>
                        <input type="hidden" name="tp" size="15" value="<?php echo $tipo ?>" />
                        Tipo:<select name="tipo" id="tipo" style="margin-top:5px;margin-left:5px;" onchange="loadData(this.value, '<?php echo $tbl_set ?>')">
            ...
            <caption  class="finder">
                <center style="border-top:double 3px #f8f8f8;">
                </center>
