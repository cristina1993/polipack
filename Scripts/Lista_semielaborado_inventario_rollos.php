...
if (isset($_GET[txt], $_GET[fecha2])) {
    ...
    $std = $_GET[estado];
    if ($fec2 == date('Y-m-d')) {
        $pro = " and (pro_codigo like '%$nm%' or pro_descripcion like '%$nm%' or m.mva_rollo like '%$nm%') and m.mva_estado='$std'";
        $cns = $Set->lista_inventario_actual($pro);
        $det = 0;
    } else {
        $txt = " and (pro_codigo like '%$nm%' or pro_descripcion like '%$nm%' or m.mvh_rollo like '%$nm%') and m.mvh_fecha='$fec2' and m.mvh_estado='$std'";
        $cns = $Set->lista_inventario_historico($txt);
        $det = 1;
    }
} else {
    $fec1 = date("Y-m-d");
    ...
    <table style="width:100%" id="tbl">
        ...
        <tbody id="tbody">
            ...
            while ($rst = pg_fetch_array($cns)) {
                if ($det == 1) {
                    $rst[mva_cantidad] = $rst[mvh_cantidad];
                    $rst[mva_peso] = $rst[mvh_peso];
                    $rst[mva_rollo] = $rst[mvh_rollo];
                    $rst[mva_estado] =$rst[mvh_estado];
                }
                if ($rst[mva_cantidad] != 0 && $rst[mva_peso] != 0) {
                    $i++;
                    if ($rst[mva_estado] == 0) {
                        $cnt_con = $rst[mva_cantidad];
                        $inv_con = $rst[mva_peso];
                        $cnt_inc = 0;
                        $inv_inc = 0;
                    } else {
                        $cnt_con = 0;
                        $inv_con = 0;
                        $cnt_inc = $rst[mva_cantidad];
                        $inv_inc = $rst[mva_peso];
                    }
                    if ($gr_id != $rst[pro_id] && $gr_lt != substr($rst[mva_rollo], 0, 7) && $n != 1) {
                        ?>
                    ...
                    $cnt_totali+=$cnt_inc;
                    $gr_id = $rst[pro_id];
                    $gr_lt = substr($rst[mva_rollo], 0, 7);
